...
if [ $keychainCount == 0 ] ; then
  ...
  security create-keychain -p $DECRYPT_PASSWORD ios-build.keychain
fi
...
security unlock-keychain -p $DECRYPT_PASSWORD ios-build.keychain
...
security set-key-partition-list -S apple-tool:,apple: -s -k $DECRYPT_PASSWORD ios-build.keychain
